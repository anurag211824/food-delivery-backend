...
export class OrderCleanupCron {
  ...
  async handleZombieOrders() {
        ...
        try {
          ...
          if (order.restaurant?.managerId) {
            this.notificationsService
              ...
              .catch((e) =>
                this.logger.error('Cleanup restaurant notification failed', e),
              );
          }

          totalCancelled++;
          this.logger.log(
            `Cleanup: cancelled zombie order ${order.id} (status: ${status}, placed: ${order.placedAt.toISOString()})`,
          );
        } catch (err) {
    ...
    if (totalCancelled > 0) {
      this.logger.warn(
        `Zombie order cleanup complete. ${totalCancelled} orders cancelled.`,
      );
    } else {
