...
export class PaymentsService {
  ...
  constructor(
    ...
    if (!keyId || !keySecret) {
      console.warn(
        '[PaymentsService] RAZORPAY_KEY_ID or RAZORPAY_KEY_SECRET not set. Payments will fail.',
      );
    }
  ...
  async createRazorpayOrder(
    ...
    try {
      ...
      this.logger.error(
        `Failed to create Razorpay order for order ${orderId}`,
        { userId, orderId, error: error.message },
      );
      throw new InternalServerErrorException(
        error.message || 'Error creating Razorpay order',
      );
    }
  }
    ...
    try {
      ...
      if (orderWithRestaurant) {
        const merchantManagerId =
          ...
          orderWithRestaurant.store?.managerId;
        const merchantEntityId =
          orderWithRestaurant.restaurantId ?? orderWithRestaurant.storeId;
        ...
        if (merchantManagerId) {
          this.notificationsService
            ...
            .catch((e) =>
              this.logger.error(
                `Failed to send push notification to merchant for order ${orderWithRestaurant.id}`,
                e,
              ),
            );
        }
      ...
      this.logger.error(
        `Error notifying restaurant manager after payment verification:`,
        error,
      );
    }
    ...
    if (event === 'payment.captured' || event === 'order.paid') {
      ...
      if (shouldNotify && orderIdToNotify) {
        try {
          ...
          if (orderWithRestaurant) {
            ...
            if (merchantManagerId) {
              this.notificationsService
                ...
                .catch((e) =>
                  this.logger.error(
                    `Failed to send push notification to merchant via webhook for order ${orderWithRestaurant.id}`,
                    e,
                  ),
                );
            }
          ...
          this.logger.error(
            `Error in webhook notifying restaurant manager for order ${orderIdToNotify}:`,
            error,
          );
        }
      }
