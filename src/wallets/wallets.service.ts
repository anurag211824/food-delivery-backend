...
export class WalletsService {
  ...
  constructor(private prisma: PrismaService) {
    ...
    if (!keyId || !keySecret) {
      this.logger.warn(
        'RAZORPAY_KEY_ID or RAZORPAY_KEY_SECRET not set. Razorpay wallet topups will fail.',
      );
    }
  ...
  async topup(userId: string, dto: TopupWalletDto) {
    ...
    try {
      ...
      throw new InternalServerErrorException(
        error.message || 'Error creating Razorpay wallet topup order',
      );
    }
  }
  ...
  async verifyTopup(userId: string, verifyDto: VerifyTopupDto) {
    ...
    if (!topupRequest || topupRequest.status === PaymentStatus.SUCCESS) {
      throw new BadRequestException(
        'Topup request not found or already processed',
      );
    }
    ...
    if (topupRequest.wallet.userId !== userId) {
      throw new BadRequestException(
        'You are not authorized to verify this topup.',
      );
    }
  ...
  async charge(
    ...
    return this.prisma.$transaction(async (prisma) => {
      ...
      const lockedWallets = await prisma.$queryRawUnsafe<{ id: string; balance: number }[]>(
  ...
  async requestWithdrawal(
    userId: string,
    dto: import('./dto/create-withdrawal.dto').CreateWithdrawalDto,
  ) {
    ...
    await this.charge(
      ...
      dto.amount,
      'WITHDRAWAL_HOLD',
      `Withdrawal request of ₹${dto.amount} — funds held pending admin approval`,
    );
  ...
  async resolveWithdrawal(
    id: string,
    dto: import('./dto/resolve-withdrawal.dto').ResolveWithdrawalDto,
  ) {
    ...
    return {
      message: `Withdrawal marked as ${dto.status}`,
      withdrawal: updated,
    };
  }
}
