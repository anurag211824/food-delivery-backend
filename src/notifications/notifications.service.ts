...
export class NotificationsService {
  ...
  async send(
    ...
    const tokens = Array.from(
      new Set(activeSessions.map((s) => s.pushToken as string)),
    );
    ...
    if (tokens.length > 0) {
      ...
      this.logger.warn(
        `No active push tokens found for user ${userId}. Push notification skipped.`,
      );
    }
  ...
  async sendToAll(title: string, body: string, type = 'SYSTEM') {
    ...
    const tokens = Array.from(
      new Set(sessions.map((s) => s.pushToken as string)),
    );
      ...
      try {
        ...
        for (const ticket of ticketChunk) {
          if (ticket.status === 'error') {
            this.logger.error(
              `Error in push ticket: ${ticket.message}`,
              ticket.details,
            );
          }
        }
        ...
        if (error.code === 'PUSH_TOO_MANY_EXPERIENCE_IDS' && error.details) {
          this.logger.warn(
            'Mixed experience IDs detected, grouping and retrying...',
          );
          ...
          for (const [experienceId, tokens] of Object.entries(details)) {
            ...
            if (projectMessages.length > 0) {
              const projectChunks =
                this.expo.chunkPushNotifications(projectMessages);
              for (const projChunk of projectChunks) {
                try {
                  await this.expo.sendPushNotificationsAsync(projChunk);
                } catch (retryError) {
                  this.logger.error(
                    `Error sending push to project ${experienceId}`,
                    retryError,
                  );
                }
              }
