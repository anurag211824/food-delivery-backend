...
export class CommunicationsProcessor extends WorkerHost {
  ...
  private async processSmsSend(job: Job<any>) {
    ...
    try {
      ...
      this.logger.error(
        `SMS send failed: ${error}. Attempt ${job.attemptsMade + 1}`,
      );
  ...
  private async processEmailSend(job: Job<any>) {
    const { communicationLogId, to, subject, template, templateData } =
      job.data;
    ...
    try {
      ...
      this.logger.log(
        `✨ Email job completed successfully. [Log ID: ${communicationLogId}]`,
      );
      return { success: true, providerMessageId };
    } catch (error) {
      this.logger.error(
        `🚨 Email job failed for ${to}: ${error}. Attempt ${job.attemptsMade + 1}/3`,
      );
