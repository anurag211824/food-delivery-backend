...
export class AdminController {
  ...
  @ApiOperation({
    ...
    description: 'Returns a paginated list of all users. Filter by role.',
  })
  ...
  @Patch('users/role')
  @ApiOperation({
    summary: "[Admin] Manually change a user's role",
    description:
      'Directly promote or demote any user. For restaurant/delivery partners, prefer the approve endpoints instead.',
  })
  ...
  @Post('users')
  @ApiOperation({
    summary: '[Admin] Create a new user account',
    description:
      'Directly create a user with a specific role. Useful for onboarding managers manually.',
  })
  ...
  @Post('stores')
  @ApiOperation({
    summary: '[Admin] Manually create a grocery store',
    description:
      'Directly onboard/create an Instamart grocery store and promote its manager.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Returns all restaurants regardless of active status, explicitly for admin listing.',
  })
  @ApiQuery({ name: 'page', required: false, example: 1 })
  @ApiQuery({ name: 'limit', required: false, example: 20 })
  @ApiQuery({
  ...
  @ApiOperation({
    ...
    description:
      'List restaurant, delivery partner, or grocery store applications. Filter by status to see what is pending.',
  })
  @ApiQuery({
    name: 'type',
    required: true,
    enum: ['restaurant', 'delivery', 'store'],
    description: 'Type of request to list',
  })
  @ApiQuery({
    name: 'status',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description: "List of requests with the applicant's user details",
  })
  @ApiQuery({ name: 'page', required: false, example: 1 })
  @ApiQuery({ name: 'limit', required: false, example: 20 })
  async listRequests(
  ...
  @ApiOperation({
    ...
    description:
      'Atomically: marks request APPROVED + sets user role to RESTAURANT_MANAGER + creates the Restaurant record.',
  })
  @ApiParam({ name: 'id', description: 'RestaurantRequest ID' })
  @ApiResponse({
    status: 200,
    description: 'Approved — Restaurant and manager role created.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Marks the request as REJECTED with an optional reason. User stays as CUSTOMER.',
  })
  @ApiParam({ name: 'id', description: 'RestaurantRequest ID' })
  @ApiBody({
    schema: {
      ...
      properties: {
        reason: {
          type: 'string',
          example: 'FSSAI license could not be verified.',
        },
      },
    },
  })
  @ApiResponse({ status: 200, description: 'Request rejected.' })
  async rejectRestaurantRequest(
    @Param('id') id: string,
  ...
  @ApiOperation({
    ...
    description:
      'Atomically: marks request APPROVED + sets user role to DELIVERY_PARTNER + creates the DriverProfile record.',
  })
  @ApiParam({ name: 'id', description: 'DeliveryPartnerRequest ID' })
  @ApiResponse({
    status: 200,
    description: 'Approved — DriverProfile and partner role created.',
  })
  @ApiResponse({ status: 400, description: 'Request is not in PENDING state' })
  @ApiResponse({ status: 404, description: 'Request not found' })
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        reason: {
          type: 'string',
          example: 'Driving license could not be verified.',
        },
      },
    },
  })
  @ApiResponse({ status: 200, description: 'Request rejected.' })
  async rejectDeliveryRequest(
    @Param('id') id: string,
    @Body('reason') reason?: string,
  ) {
  ...
  @ApiOperation({
    ...
    description:
      'Atomically: marks request APPROVED + sets user role to STORE_MANAGER + creates the Store record.',
  })
  @ApiParam({ name: 'id', description: 'StoreRequest ID' })
  @ApiResponse({
    status: 200,
    description: 'Approved — Store and manager role created.',
  })
  @ApiResponse({ status: 400, description: 'Request is not in PENDING state' })
  @ApiResponse({ status: 404, description: 'Request not found' })
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        reason: {
          type: 'string',
          example: 'GST documentation could not be verified.',
        },
      },
    },
  })
  ...
  @ApiOperation({
    ...
    description:
      'Returns total users, restaurants, orders, revenue, active drivers, and pending requests — all in one call.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Paginated list of all orders across the platform. Filter by status.',
  })
  @ApiQuery({
    name: 'status',
    required: false,
    description: 'Filter by order status (e.g. PLACED, DELIVERED, CANCELLED)',
  })
  @ApiQuery({ name: 'page', required: false, example: 1 })
  @ApiQuery({ name: 'limit', required: false, example: 20 })
  async getAllOrders(
  ...
  @ApiOperation({
    ...
    description:
      'Issues a custom/partial refund directly to the customer wallet and records the refund.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Returns a paginated list of all system refunds (auto & manual).',
  })
  @ApiQuery({ name: 'page', required: false, example: 1 })
  @ApiQuery({ name: 'limit', required: false, example: 20 })
  ...
  @ApiOperation({
    ...
    description:
      'Returns a paginated list of all restaurant settlements. Can filter by status (PENDING, PAID).',
  })
  @ApiQuery({
    name: 'status',
    required: false,
  ...
  @ApiOperation({
    ...
    description:
      'Manually runs the settlement calculation engine for the previous day.',
  })
