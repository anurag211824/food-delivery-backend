...
export class AppConfigService {
  ...
  async getReferralPolicy(): Promise<ReferralPolicy> {
    ...
    return {
      ...
      referrerReward: parseFloat(
        map['referrer_reward'] ?? String(DEFAULTS.referrerReward),
      ),
      referredReward: parseFloat(
        map['referred_reward'] ?? String(DEFAULTS.referredReward),
      ),
    };
  }
