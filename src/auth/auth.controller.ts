...
export class AuthController {
  ...
  @ApiOperation({
    ...
    description:
      'Create a new account using email + password. Optionally pass an `invitedByCode` to attribute a referral.',
  })
  @ApiBody({ type: SignUpDto })
  @ApiResponse({
    status: 201,
    description:
      'Account created. Returns the new user object and session token.',
    schema: {
      example: {
  ...
  @ApiOperation({
    ...
    description:
      'Authenticate using email and password. Returns a session token to use as Bearer on protected endpoints.',
  })
  @ApiBody({ type: SignInDto })
  @ApiResponse({
    status: 200,
    description:
      'Login successful. Use the returned `token` as `Authorization: Bearer <token>`.',
    schema: {
      example: {
        token: 'eyJhbGciOiJIUzI1NiJ9...',
        user: {
  ...
  @ApiOperation({
    summary: 'Sign out',
    description:
      'Invalidate the current session. Pass your Bearer token in the Authorization header.',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Initiate Google OAuth. Returns a redirect URL to open in the browser.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Sends a 6-digit OTP via SMS to the provided phone number. Call this first, then verify with the `/verify` endpoint.',
  })
  @ApiBody({
    schema: {
      type: 'object',
  ...
  @ApiOperation({
    ...
    description:
      'Verify the OTP sent to the phone number. Creates an account if one does not exist. Optionally pass `invitedByCode` to attribute a referral.',
  })
  @ApiBody({
    schema: {
      type: 'object',
      ...
      properties: {
        ...
        invitedByCode: {
          ...
          example: 'PRIYA4823',
          description:
            '(Optional) Referral code from a friend — only applied on first signup',
        },
      },
    },
  })
  ...
  @ApiOperation({
    ...
    description:
      "Check if a referral code is valid before the user completes signup. Returns the referrer's first name on success.",
  })
  @ApiParam({
    name: 'code',
    example: 'PRIYA4823',
    description: 'The referral code to check (case-insensitive)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 404,
    description: 'Referral code not found / invalid',
  })
  ...
  @ApiOperation({
    ...
    description:
      "Returns the full profile of the logged-in user including their shareable referral code and how many friends they've referred.",
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 401,
    description: 'Not authenticated — send Bearer token',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Internal route handled by Better Auth for Google OAuth callbacks and deep links. Do not call directly.',
  })
